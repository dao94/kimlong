    ...
    .controller('SlideController', [
        '$scope','$http', '$restful','$modal','growl', 'FileUploader', 'CSRF_TOKEN',  function ($scope, $http, $restful, $modal,growl, FileUploader, CSRF_TOKEN) {
            ...
            $scope.change_status = function(list) {
                ...
                $http({
                    method  : 'POST',
                    url     : base_url+'slide/changestatus',
                    data    : data,
                    dataType: 'json'
        	...
        	$scope.loadPage = function (stt){
               var stt = stt || "";
        		$http.get(base_url + 'slide?limit='+$scope.itemsPerPage+'&page='+$scope.currentPage+'&stt='+stt).success(function (resp){
            ...
            $scope.modalOpen_add = function (size) {
                ...
                modalInstance = $modal.open({
                    templateUrl: 'views/admin/slide/add.html',
                    size: size,
                    controller: function ($scope, $modalInstance, growl, $http, FileUploader, CSRF_TOKEN) {
                        ...
                        $scope.add_item = function() {
                            $scope.loading = true;
                            $scope.disable = true;
                            var data = {name:$scope.name,images:$scope.image.images,link:$scope.link,alt:$scope.alt};
                            $http({
                                method  : 'POST',
                                url     : base_url+'slide/post',
                                data    : data,
                                dataType: 'json'
                                }).success(function (result){
                                    if(result.message == 'Done') {
            ...
            $scope.modalOpen_update = function (rs,index,size) {
                $scope.news = {};
                modalInstance = $modal.open({
                    templateUrl: 'views/admin/slide/edit.html',
                    size: size,
                    controller: function ($scope, $modalInstance, growl, $http, FileUploader, CSRF_TOKEN) {
                        /*Upload file images*/ 
                        $scope.row = {};
                        ...
                        $scope.edit_item = function() {
                            $scope.loading = true;
                            $scope.disable = true;
                            var data = {id:rs.id,name:$scope.row.name,images:$scope.row.images,alt:$scope.row.alt};
                            $http({
                                method  : 'POST',
                                url     : base_url+'slide/push',
                                data    : data,
                                dataType: 'json'
                                }).success(function (result){
                                    if(result.message == 'Done') {
                ...
                if(confirm("Bạn muốn xóa ảnh này ?")){
                    ...
                    $http({
                        ...
                        dataType: 'json'
                            ...
                            if(result.message == 'Done') {
                                growl.success("Xóa thành công slide !");
                                $scope.disable = false;
                                $scope.loading = false;
                                $scope.loadPage();
                            }else {

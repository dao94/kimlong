...
class ProductController extends Controller {
	...
	*/
	...
	public function postPost(Request $request){
		...
		if(isset($name) && !empty($name) && !empty($category) && !empty($image)  && !empty($des)) {
			if(in_array($name,$col_data)) {
				...
				$data = ['id' => $LastInsertId,
							...
							'description' => $des,
							'content'     => $content,
							'status'      => 1,//2 không kích hoạt,1 là đã kích hoạt
	...
	public function postPush(Request $request) {
		...
		$keywords    = $request->input('keywords');
		$col_data    = DB::table('product')->lists('name');
		$_category   = DB::table('category')->where('id','=',$category_id)->pluck('name');
		if(!empty($name)) {
